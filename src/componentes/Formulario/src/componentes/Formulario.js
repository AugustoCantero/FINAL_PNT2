...
export default {
  ...
  methods: {
    ...
    async enviar() {
      ...
      const usuarioNotas = await this.getUsuarioNotas(datos.nombre)
    ...
    async getUsuarioNotas(nombre) {
      try {
        const response = await axios.get(`https://66562f2f9f970b3b36c490cd.mockapi.io/api/usuarios?nombre=${nombre}`)
